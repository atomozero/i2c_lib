...
status_t i2c_read(void* cookie, off_t pos, void* buffer, size_t* length) {
    dprintf(STDOUT_FILENO, "i2c_driver: i2c_read() - Posizione: %lld, Lunghezza: %zu\n", pos, *length);
...
status_t i2c_write(void* cookie, off_t pos, const void* buffer, size_t* length) {
    dprintf(STDOUT_FILENO, "i2c_driver: i2c_write() - Posizione: %lld, Lunghezza: %zu\n", pos, *length);
...
status_t init_driver() {
    ...
    if (publish_device(I2C_DEVICE_PATH, &i2c_hooks) != B_OK) {
...
void uninit_driver() {
    ...
    unpublish_devices(I2C_DEVICE_PATH);  // Correzione della funzione
}
